        ...
        .then((json) => {
          ...
          client.db.execute(gettime).then(function ([results]) {
            ...
            if (interaction.options._subcommand == "couleur") {
              ...
              if (cooldownco.has(interaction.user.id)) {
                ...
                const timeLeft =
                  cooldownco.get(interaction.user.id) - Date.now();
                ...
                setTimeout(
                  () => cooldownco.delete(interaction.user.id),
                  timeco
                ); // <- I don't remember what it does but it's needed
                interaction.reply({
                  content: `<@${interaction.user.id}>`,
                  embeds: [
          ...
          if (interaction.options._subcommand == "nombre") {
            ...
            if (cooldownnum.has(interaction.user.id)) {
              // if user on cooldown
              const timeLeft =
                cooldownnum.get(interaction.user.id) - Date.now();
              const converted = convertMS(timeLeft); // Changes the ms to time
              //add message here if code
              interaction.reply({
                content: `<@${interaction.user.id}>`,
              ...
              setTimeout(
                () => cooldownnum.delete(interaction.user.id),
                timenum
              );
                ...
                var bb = `SELECT * FROM tblclients WHERE email='${rows[0].email}'`;
                  ...
                  return interaction.reply({
                    ...
                    embeds: [
                      new EmbedBuilder()
                        .setDescription(
                          `Vous disposez de ${rows[0].balance} points sur votre compte discord et de ${rowsss[0].credit} points sur le manager.`
                        )
                        .setColor("Green"),
                    ],
                    ephemeral: false,
                ...
                var bb = `SELECT * FROM users WHERE email='${rows[0].email}'`;
                  ...
                  return interaction.reply({
                    ...
                    embeds: [
                      new EmbedBuilder()
                        .setDescription(
                          `Vous disposez de ${rows[0].balance} points sur votre compte discord et de ${rowsss[0].money} points sur le manager.`
                        )
                        .setColor("Green"),
                    ],
                    ephemeral: false,
