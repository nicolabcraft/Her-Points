    ...
    if (config.Type == "1") {
        ...
        if (config.Type == "2") {
            ...
            if (config.Type == "") {
                console.log(
                    "\n" +
                        `[Erreur] Vous n'avez pas précisé sur quelle plateforme vous voulez que le bot agisse.`
                            .red
                );
                process.exit();
            } else {
            ...
            if (error) {
                console.log(
                    "Vérifiez les informations de connexion à la base de données."
                        .red,
                    "\nVérifiez également que vous avez correctement installé votre CMS."
                        .red
                );
                process.exit();
            }
...
client.once("ready", async () => {
    ...
    if (config.Type == "1") {
        ...
        connection.query(
            ...
            function (error, results, fields) {
                if (
                    ...
                    console.log(
                        "\n" +
                            `[INSTALLATION-AUTOMATIQUE] La base de données n'est pas encore installée, je vais donc l'installer.`
                                .red
                    );
                    ImportUsersTable =
                        "CREATE TABLE users ( id varchar(255) NOT NULL PRIMARY KEY, email text NOT NULL, balance text NOT NULL, time text NOT NULL, autotransfert text NOT NULL, dailytime text NOT NULL, weeklytime text NOT NULL, monthlytime text NOT NULL, dailyremind text NOT NULL, weeklyremind text NOT NULL, monthlyremind text NOT NULL, daily text NOT NULL, weekly text NOT NULL, monthly text NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;";
                    connection.query(
                        ImportUsersTable,
                        function (error, results, fields) {
                            ImportConfigTable =
                                "CREATE TABLE botadminconfig ( multiplesconnexions text NOT NULL DEFAULT 'yes') ENGINE=InnoDB DEFAULT CHARSET=latin1;";
                            connection.query(
                                ImportConfigTable,
                                function (error, results, fields) {
                                    if (error) {
                                        console.log(
                                            "\n" +
                                                `[INSTALLATION-AUTOMATIQUE] Erreur rencontrée lors de l'installation de la base de données.`
                                                    .red
                                        );
                                        console.log("\n" + error.message);
                                    } else {
                                        ...
                                        console.log(
                                            "\n" +
                                                `[INSTALLATION-AUTOMATIQUE] Base de données installée!`
                                                    .green
                                        );
                                    }
                                }
        ...
        if (config.Type == "2") {
            ...
            connection.query(
                GetRequestIfAlreadyInstalledAzuriom,
                function (error, results, fields) {
                    if (
                        error &&
                        error.message ==
